...
def predict(
    *,
    ...
    with path_output.open("w") as outf, path_input.open() as inf, torch.no_grad():
        for _input in get_input(inf, batch_size):
            ...
            predictions = torch.nn.functional.softmax(outputs.logits, dim=-1).detach().cpu().numpy()  # type: ignore
...
def evaluate(
    *,
    path_model: Path,
    path_data_list: typing.List[Path],
    path_output: Path,
    task: str,
    max_length: int,
):
    ...
    tokenizer = AutoTokenizer.from_pretrained(path_model)
    test_texts_a, test_texts_b, test_labels = read_data(path_data_list, "test", task)

    with path_output.open("w") as outf, torch.no_grad():
        ...
        for (text_a, text_b, gold_label_idx) in zip(test_texts_a, test_texts_b, test_labels):
            if text_b is None:
                source = tokenizer.batch_encode_plus([text_a], return_tensors="pt")
            else:
                source = tokenizer.batch_encode_plus([[text_a, text_b]], return_tensors="pt")
            source.to(device)
            outputs = model(
                input_ids=source["input_ids"],
                token_type_ids=source["token_type_ids"],
...
def main(
    *,
    ...
    train_texts_a, train_texts_b, train_labels = read_data(path_data_list, "train", task)
    train_encodings = tokenizer(
        text=train_texts_a, text_pair=train_texts_b, truncation=True, padding="max_length", max_length=max_length
    )
    train_dataset = MyDataset(train_encodings, train_labels)

    eval_texts_a, eval_texts_b, eval_labels = read_data(path_data_list, "dev", task)
    eval_encodings = tokenizer(
        text=eval_texts_a, text_pair=eval_texts_b, truncation=True, padding="max_length", max_length=max_length
    )
    ...
    training_args = TrainingArguments(
        ...
        logging_dir=str(path_log),
        logging_steps=log_step,
    )

    label_map: typing.Dict[int, str] = {i: label for i, label in enumerate(TASK2LABELS[task])}
    ...
    model = BertForSequenceClassification.from_pretrained(base, config=config)

    trainer = Trainer(model=model, args=training_args, train_dataset=train_dataset, eval_dataset=eval_dataset)

    trainer.train()  # type: ignore
    trainer.save_model()  # type: ignore
    if trainer.is_world_process_zero():  # type: ignore
...
def get_opts() -> argparse.Namespace:
    ...
    oparser.add_argument("--base", default=default_base)
